...
class SalesPerformReportXLS(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, patients):
        ...
        sheet.write(row, col + 3, 'TARGET', format21)
        sheet.write(row, col + 4, 'MTD ACH.', format21)
        sheet.write(row, col + 5, 'ACHIEVEMENT %', format21)
        sheet.write(row, col + 6, 'SHORTFALL/SURPLUS', format21)
        ...
        for emp in data['emp_list']:
            ...
            sheet.write(row, col + 3, emp['target'], font_size_8_r)
            sheet.write(row, col + 4, emp['achievement'], font_size_8_r)
            sheet.write(row, col + 5, emp['ach_percent'], font_size_8)
            sheet.write(row, col + 6, emp['shortfall'], font_size_8_r)
            sheet.write(row, col + 7, emp['short_percent'], font_size_8)
